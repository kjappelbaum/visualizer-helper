...
define(['src/util/ui'], function(UI) {
    ...
    function showUserInfo(user) {
        ...
        html+='<table>';
        if (user.userId) html+=`<tr>
                <th>User ID</th>
                <td>${user.userId}</td>
            </tr>`;

        if (user.firstName) html+=`<tr>
                <th>Firstname</th>
                <td>${user.firstName}</td>
            </tr>`;


        if (user.lastName) html+=`<tr>
                <th>Lastname</th>
                <td>${user.lastName}</td>
            </tr>`;


        if (user.groupName) html+=`<tr>
                <th>Group</th>
                <td>${user.groupName}</td>
            </tr>`;

        if (user.phoneNumber) html+=`<tr>
                <th>Phone number</th>
                <td>${user.phoneNumber}</td>
            </tr>`;


        if (user.roomNumber) html+=`<tr>
                <th>Room number</th>
                <td>${user.roomNumber}</td>
            </tr>`;

        if (user.postalAddress) html+=`<tr>
                <th>Room number</th>
                <td>${user.postalAddress}</td>
            </tr>`;

        if (user.allGroups && Array.isArray(allGroups)) html+=`<tr>
                <th>Groups</th>
