...
define(['https://www.lactame.com/github/adobe-webplatform/Snap.svg/84fbff7d512c8145c522b71fc9c872cb0bcae49a/dist/snap.svg-min.js'], function (Snap) {
    ...
    function getSVG(sequence, analysisResult, options) {
        ...
        var residues=[];
        var mfParts=sequence.replace(/ /g,'').replace(/([A-Z])(?=[A-Z])/g,"$1 ").split(/ /);
...
// we calculate all the lines based on the results
        for (let result of results) {
            ...
            if ("abc".indexOf(firstPart.charAt(0))>-1) { // n-terminal fragment
...
// where it should be drawn as well and the maximal number of lines
        ...
        var height=rowHeight * (line+1) + 50 + verticalShiftForTerminalAnnotations;
            ...
            for (var result of results) {
                if (result.internal) {
                    ...
                    for (var line=fromResidue.line; line<=toResidue.line; line++) {
                        ...
                        drawLine.attr({stroke:result.color,'stroke-width':strokeWidth});
