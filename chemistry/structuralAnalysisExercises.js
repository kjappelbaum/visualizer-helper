...
async function fetchData(query=undefined, url='https://couch.cheminfo.org/cheminfo-public/668677b6432fb3fde76305cfe706856d') {
    var ca=new CA(url);
    ...
    files=files.map( a => {
        ...
        if (filename.match(/.*\.mol/)) {
            var parts=[filename.replace(/\..*/,''), 'mol'];
        } else {
